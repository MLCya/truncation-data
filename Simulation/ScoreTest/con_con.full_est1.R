##    to calculate the type 1 error  and power  for qz=(1,z1,z2),z1~B(1,0.5), z2~N(0,1)     ##
...
gen_obs <- function(beta,rate.G,tau,n_big){
  ...
  theta <- NULL
  for (i in 1:n_big) {
    hi <- exp(beta[1]*z0[i]+beta[2]*z1[i]+beta[3]*z2[i] + nu[i]*sqrt(tau))
    x[i] <- rexp(1,rate = hi)
    theta[i] <- hi
  }
  
  ind <- (x>y)
  obs <- data.frame(obs.x = x[ind],obs.y = y[ind],z0.obs=z0[ind],z1.obs=z1[ind],z2.obs=z2[ind],theta.obs = theta[ind])
  return(obs)
}



# full likelihood esimation procedure:    parametric model   theta = h(z,beta) = exp(beta0 +beta1*z1+beta2*z2)
maxlike.par <- function(obs){
  ...
  prof.lik <- function(beta){  ###profile loglikelihood of beta
    ...
    fn.iner <- function(gam){ ## gam is a reparameter
      ...
      if(max(d) * min(d) < 0){
        ...
        low= -1/max(d) + eeps
        up = -1/min(d) - eeps 
        lab = uniroot(fl, c(low,up),extendInt = c("no"))$root
      }
  ...
  if(max(d_Phat)*min(d_Phat)<0){
    ...
    lab_Phat = uniroot(fl, c(low,up),extendInt = c("no"))$root
  }
...
cb_est <- function(obs){
  ...
  hat_N   <- hat_par[1]
  hat_alpha <- hat_par[2]
  hat_beta <- matrix(hat_par[3:5],ncol = 3,dimnames = NULL)
  hat_lab  <-  hat_par[6]
  
  hat_theta <- NULL
  hat_com <- NULL
  for (i in 1:n) {
    qz_i <- c(z0[i],z1[i],z2[i])
    hat_theta[i] <-  exp(hat_beta[1]*z0[i]+hat_beta[2]*z1[i]+hat_beta[3]*z2[i])
...
inV33_est <- function(obs){
  ...
  for (i in 1:n) {
    ...
    hat_coef <- c( hat_coef,  (int_val - sec_val )*hat_theta[i]^2*hat_pi)
  }  
  
  V1_est  <- -1*sum(hat_coef)
  Vz1_est <- -1*sum(hat_coef*z1)
  Vz2_est <- -1*sum(hat_coef*z2)
  
  Vz1s_est <- -1*sum(hat_coef*z1^2)## V_{z_1^2}
  Vz2s_est <- -1*sum(hat_coef*z2^2)
  Vz12_est <- -1*sum(hat_coef*z1*z2)##V_{z_1 z_2}
  
  V_s <- Vz1_est*Vz2s_est - Vz2_est*Vz12_est
  V_t <- Vz1s_est*Vz2s_est - Vz12_est^2
  V_k <- V1_est*Vz2s_est - Vz2_est^2
  V_l <- V_k*V_t-V_s^2
  
  ############ The  components of V_{33}^{-1}  ########################
  inV_11 <- V_t*Vz2s_est/V_l
  inV_12 <- -V_s*Vz2s_est/V_l
  inV_13 <- (V_s*Vz12_est - V_t*Vz2_est)/V_l
  inV_22 <- V_k*Vz2s_est/V_l
  inV_23 <- (V_s*Vz2_est - V_k*Vz12_est)/V_l
  inV_33 <- (1+  (V_t*Vz2_est^2 - 2*V_s*Vz2_est*Vz12_est+V_k*Vz12_est^2 )/V_l)/Vz2s_est
  
  com <- c(inV_11,inV_12,inV_13,inV_12,inV_22,inV_23,inV_13, inV_23,inV_33)
  matrix(com,ncol = 3,byrow = T,dimnames = NULL)
}
...
sigmas2_est <- function(obs){
  ...
  cv   <- c(hat_cb[1]*hat_inV33[1,1]+hat_cb[2]*hat_inV33[2,1]+hat_cb[3]*hat_inV33[3,1],
            hat_cb[1]*hat_inV33[1,2]+hat_cb[2]*hat_inV33[2,2]+hat_cb[3]*hat_inV33[3,2],
            hat_cb[1]*hat_inV33[1,3]+hat_cb[2]*hat_inV33[2,3]+hat_cb[3]*hat_inV33[3,3])
  ...
  for (i in 1:n) {
    ...
    hat_theta[i] <- exp(hat_beta[1]*z0[i]+hat_beta[2]*z1[i]+hat_beta[3]*z2[i])
    hat_pi <- 1/n * 1/(1+hat_lab*(f_b(y[i], hat_theta[i]) - hat_alpha )   )
    cvq <- cv[1] + cv[2]*z1[i] + cv[3]*z2[i]
    #cvq <- sum( cv * qz_i)
    fn <- function(x){
      ((hat_theta[i]*x-2)*hat_theta[i]*x+(1-hat_theta[i]*x)*(1-cvq))^2/(hat_theta[i]*exp(hat_theta[i]*x))
      #(f2_s(x,hat_theta[i])*hat_theta[i]+f1_s(x,hat_theta[i])*(1-cvq))^2/f_s(x,hat_theta[i] )
    }
...
U_ts <- function(obs){
  ...
  for (i in 1:n) {
    ...
    sec_val <- (f2_b(y[i],hat_theta[i])*hat_theta[i] + f1_b(y[i],hat_theta[i]))*exp(hat_theta[i]*y[i])
...
power_case1 <- data.frame(cbind(ty1_error,pw1,pw2,pw3,pw4,pw5), row.names = c("1%","5%"))
